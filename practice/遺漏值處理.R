...
library(mice)
md.pattern(msleep) # missing data pattern

library(mice)
md.pattern(newdata) # missing data pattern

library(dplyr)
library(VIM)
imputeData <- kNN(unlist(starwars))



# Missing Value Treatment
# 英文：http://r-statistics.co/Missing-Value-Treatment-With-R.html
# 中文：https://www.jamleecute.com/missing-value-treatment-遺失值處理/
# 判斷遺失值在整體資料中所佔的比例
...
# 實務上不能使用該筆資料，直接忽略遺漏值後的線性模型

# 2. 刪除特徵變數法
# 直接忽略該欄位
# 「變數重要性」與「遺失資料數」之間的抉擇。

# 3. 可以使用Hmisc套件的impute()來填補特定值（平均、中位、自訂值）

install.packages("Hmisc")
library(Hmisc)
Hmisc::impute(BostonHousing$ptratio, mean)  # 以平均數填補
